...
public class ZooKeeperPeerServerImpl implements ZooKeeperPeerServer {
            ...
            while (!shutdown) {
                ...
                switch (state) {
                    case LOOKING:
                        ...
                        break;
                    case LEADING:
                        if(rrl == null) runAsLeader();
                        break;
                    case FOLLOWING:
                        if(jrf == null) runAsFollower();
                        break;
                }
            }
        }
    ...
    {
        Util2.printReplace("Server port:" + getMyPort() + ", is now LEADING", 12_05);
        rrl = new RoundRobinLeader(peerIDtoAddress, getMyPort());
        new Thread(rrl).start();
    }

    private void runAsFollower()
    {
        Util2.printReplace("Server port:" + getMyPort() + ", is now FOLLOWING", 12_05);
