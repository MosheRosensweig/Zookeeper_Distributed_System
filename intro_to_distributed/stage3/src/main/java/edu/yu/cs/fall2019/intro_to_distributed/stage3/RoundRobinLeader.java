...
public class RoundRobinLeader implements Runnable
{
    ...
    private GateWay gateWay;//todo - remove in stage 4

    public RoundRobinLeader(HashMap<Long, InetSocketAddress> peerIDtoAddress, int MY_PORT)
    {
        this.peerIDtoAddress = peerIDtoAddress;
        this.MY_PORT = MY_PORT;
    }

    public RoundRobinLeader(HashMap<Long, InetSocketAddress> peerIDtoAddress, int MY_PORT, LinkedBlockingQueue<Message> fromGateway)
    {
        this.peerIDtoAddress = peerIDtoAddress;
        this.MY_PORT = MY_PORT;
        this.messagesFromGateway = fromGateway;
    }

    public void shutdown()
    {
        Util2.printReplace("Shutting down round robin leader");
        //todo - shutdown everything
        socketHandler.shutdown();
        roundRobinFeeder.shutdown();
    ...
    private void setupRoundRobinFeeder()
    {
        ...
        new Thread(roundRobinFeeder).start();
    }

    /**
     * This is temporary for stage 3
     */
    private void setupGateway()
    {
        GateWay gateWay = new GateWay();
        gateWay.setNewLeader(MY_PORT, HOST);
